...
export const Main = () => {
  ...
  const dappTokenAddress: string = chainId
    ? networkMapping[String(chainId)]["ProjectToken"][0]
    : constants.AddressZero;
  const wethTokenAddress: string = chainId
    ? brownieConfig["networks"][networkName]["weth_token"]
    : constants.AddressZero;
  const daiTokenAddress: string = chainId
    ? brownieConfig["networks"][networkName]["fau_token"]
    : constants.AddressZero;
  const cETHTokenAddress: string = chainId
    ? brownieConfig["networks"][networkName]["cETH"]
    : constants.AddressZero; //not supported in deployed contract on kovan
  const LINKTokenAddress: string = chainId
